...
class CheatTests extends WordSpec with Matchers with Cheat {
  ...
  "The GroceryItem case class" should {
    ...
    "blargh" in {
      case class Image(data: Array[Byte]) {
        override def equals(other: Any): Boolean = this.canEqual(other) && other.asInstanceOf[Image].data.deep == this.data.deep
      }
      val i1 = Image(Array[Byte](1,2,3))
      val i2 = Image(Array[Byte](1,2,3))
      i1 shouldEqual i2
    }
  }
}
